...
def _gather_values(formula_spec: dict[str, Any], inputs: list[dict[str, Any]]) -> tuple[dict[str, float] | None, str | None, dict[str, Any] | None]:
    ...
    if all_same_unit:
        values = {input_metric: float(item["value"]) for input_metric, item in zip(formula_spec["inputs"], inputs)}
...
def _calc_expression(name: str, values: dict[str, float]) -> float:
    if name == "gross_profit": return values["revenue"] - values["cost_of_revenue"]
    if name == "ebitda": return values["ebit"] + values["depreciation"] + values["amortisation"]
    if name == "net_debt": return values["total_debt"] - values["cash_and_equivalents"]
    if name == "net_cash": return values["cash_and_equivalents"] - values["total_debt"]
    if name == "working_capital": return values["current_assets"] - values["current_liabilities"]
    if name == "free_cash_flow": return values["operating_cash_flow"] - values["capital_expenditure"]
    if name == "ebitda_margin": return values["ebitda"] / values["revenue"] * 100
    if name == "ebit_margin": return values["ebit"] / values["revenue"] * 100
    if name == "gross_margin": return values["gross_profit"] / values["revenue"] * 100
    if name == "net_margin": return values["net_income"] / values["revenue"] * 100
    if name == "current_ratio": return values["current_assets"] / values["current_liabilities"]
    if name == "quick_ratio": return (values["cash_and_equivalents"] + values["short_term_investments"] + values["accounts_receivable"]) / values["current_liabilities"]
    if name == "asset_turnover": return values["revenue"] / values["total_assets"]
    if name == "receivable_days": return values["accounts_receivable"] / values["revenue"] * 365
    if name == "inventory_days": return values["inventory"] / values["cost_of_revenue"] * 365
    if name == "payable_days": return values["accounts_payable"] / values["cost_of_revenue"] * 365
    if name == "cash_conversion_cycle": return values["receivable_days"] + values["inventory_days"] - values["payable_days"]
    if name == "debt_to_equity": return values["total_debt"] / values["shareholders_equity"]
    if name == "debt_to_ebitda": return values["total_debt"] / values["ebitda"]
    if name == "net_debt_to_ebitda": return values["net_debt"] / values["ebitda"]
    if name == "interest_coverage": return values["ebit"] / values["finance_cost"]
    if name == "fcf_margin": return values["free_cash_flow"] / values["revenue"] * 100
    if name == "cfo_to_ebitda": return values["operating_cash_flow"] / values["ebitda"] * 100
    if name == "cfo_to_pat": return values["operating_cash_flow"] / values["net_income"] * 100
...
def _result_unit(formula_spec: dict[str, Any], inputs: list[dict[str, Any]], unit_mode: str | None) -> str | None:
    if formula_spec.get("unit"): return formula_spec["unit"]
...
def _resolve(conn, entity: str, metric: str, period: str, statement: str | None,
    ...
    if requested in stack:
        return {"status": "CONFLICTED", "metric": requested, "period": period, "reason": "cyclic rule dependency"}
    direct = _direct_lookup(conn, entity, requested, period, statement, consolidated)
    if direct.get("status") == "REPORTED": return direct
    if requested not in _RULES.get("formulas", {}): return direct
    ...
    for input_metric in formula_spec["inputs"]:
        ...
        if item.get("status") not in {"REPORTED", "DERIVED"}:
            return {"status": item.get("status", "UNAVAILABLE"), "metric": requested, "period": period,
                    "reason": f"missing or ambiguous input: {input_metric}", "formula": formula_spec["expression"], "inputs": inputs}
    values, unit_mode, error = _gather_values(formula_spec, inputs)
    if error is not None:
        return {"status": "UNAVAILABLE", "metric": requested, "period": period, "reason": error["reason"],
    ...
    if requested in {"debt_to_ebitda", "net_debt_to_ebitda"} and values.get("ebitda", 1) <= 0:
        return {"status": "UNAVAILABLE", "metric": requested, "period": period,
                "reason": "EBITDA is zero or negative; leverage multiple is not meaningful", "formula": formula_spec["expression"], "inputs": inputs}
    if requested == "net_cash" and values.get("total_debt", 0) > values.get("cash_and_equivalents", 0):
        return {"status": "UNAVAILABLE", "metric": requested, "period": period,
                "reason": "debt exceeds cash; report net_debt instead of net_cash", "formula": formula_spec["expression"], "inputs": inputs}
    for denom in ("revenue", "current_liabilities", "shareholders_equity", "finance_cost", "ebitda", "net_income", "total_assets", "cost_of_revenue"):
        if denom in values and denom in formula_spec["expression"] and values[denom] == 0:
            return {"status": "UNAVAILABLE", "metric": requested, "period": period,
                    "reason": f"denominator {denom} is zero", "formula": formula_spec["expression"], "inputs": inputs}
    try:
        result = _calc_expression(requested, values)
    except (ZeroDivisionError, ValueError):
        return {"status": "UNAVAILABLE", "metric": requested, "period": period,
                "reason": "calculation could not be completed", "formula": formula_spec["expression"], "inputs": inputs}
    if not math.isfinite(result):
        return {"status": "UNAVAILABLE", "metric": requested, "period": period, "reason": "non-finite calculation result", "inputs": inputs}
    source_pages = {str(x.get("metric")): x.get("source_page") for x in inputs}
    source_types = {str(x.get("metric")): x.get("source_type") for x in inputs if x.get("source_type")}
    return {
        "status": "DERIVED", "metric": requested, "value": round(result, 6), "unit": _result_unit(formula_spec, inputs, unit_mode),
        "period": period, "statement": statement, "consolidated": consolidated,
        "confidence": _confidence_level(inputs), "formula": formula_spec["expression"], "inputs": inputs,
        "source_pages": source_pages, "source_types": source_types,
    }
...
# Remaining calculations are intentionally unchanged and continue to use _resolve.
...
def calculate_growth(conn, entity: str, metric: str, current_period: str,
    ...
    mapping = {"revenue": "revenue_growth", "ebitda": "ebitda_growth", "net_income": "pat_growth", "ebit": "ebit_growth"}
    growth_metric = mapping.get(base)
    if growth_metric is None:
        return {"status": "UNAVAILABLE", "metric": base, "reason": "growth rule not defined for this metric"}
    ...
    if any(x.get("status") not in {"REPORTED", "DERIVED"} for x in inputs):
        return {"status": "UNAVAILABLE", "metric": growth_metric, "reason": "current or prior value unavailable", "inputs": inputs}
    if not units_compatible(current.get("unit"), prior.get("unit")):
        return {"status": "UNAVAILABLE", "metric": growth_metric, "reason": "current and prior values use incompatible currencies", "inputs": inputs}
    ...
    if prior_value == 0 or prior_value < 0:
        return {"status": "UNAVAILABLE", "metric": growth_metric, "reason": "prior-period denominator is zero or negative", "inputs": inputs}
    result = (current_value / prior_value - 1) * 100
    return {
        "status": "DERIVED", "metric": growth_metric, "value": round(result, 6), "unit": "%", "period": current_period,
        "formula": f"({current_period} / {prior_period} - 1) * 100", "inputs": inputs,
