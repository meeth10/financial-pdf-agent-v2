...
def _search_india(company: str, period: str | None, exchange: str, consolidated: bool) -> tuple[list[dict[str, Any]], list[str]]:
    ...
    def rank(item: dict[str, Any]) -> tuple[int, int, str]:
        ...
        exchange_score = 1 if (consolidated and item.get("exchange") == "BSE") else 0
        return (scope_score, exchange_score, str(item.get("filing_date") or ""))
...
def get_or_fetch_financials(entity: str, period: str | None = None,
    ...
    """Store-first Indian financial retrieval with conservative scope handling."""
    ...
    try:
        ...
        summary = auto_ingest(
            str(pdf_path), entity, doc_type, selected_period, selected_period, _db_path(),
