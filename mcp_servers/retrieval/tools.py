...
def _cache_path(url: str, exchange: str, format_name: str = "PDF") -> Path:
    ...
    source_name = Path(url.split("?", 1)[0]).name or ("financial_result.html" if format_name == "IXBRL" else "financial_result.pdf")
    if format_name == "IXBRL" and not source_name.lower().endswith(('.html', '.htm', '.xhtml', '.xml')):
        source_name += ".html"
...
def _download_attachment(url: str, exchange: str, format_name: str) -> Path:
    _throttle()
    ...
    if format_name == "PDF":
        ...
        if "html" not in content_type and "xml" not in content_type and b"<html" not in response.content[:1024].lower():
...
def _search_india(company: str, period: str | None, exchange: str, consolidated: bool) -> tuple[list[dict[str, Any]], list[str]]:
    ...
    def filing_timestamp(item: dict[str, Any]) -> str:
        return str(item.get("filing_date") or "")
    ...
    def rank(item: dict[str, Any]) -> tuple[int, int, str]:
        ...
        format_score = 2 if str(item.get("format", "PDF")).upper() == "IXBRL" else 1
        return (scope_score, format_score, filing_timestamp(item))
...
def get_or_fetch_financials(entity: str, period: str | None = None,
    ...
    """Store-first Indian financial retrieval; prefer exchange iXBRL over PDF when available."""
    ...
    try:
        ...
        if format_name == "IXBRL":
            summary = ingest_xbrl(
                content_path,
                entity=entity,
                fiscal_year=selected_period,
